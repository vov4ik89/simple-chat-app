...
const onConnected = () => {
    ...
    stompClient.send("/app/chat.newUser",
        {},
        JSON.stringify({user: username, type: 'CONNECT'})
    )
...
const onMessageReceived = (payload) => {
    ...
    if (message.type === 'CONNECT') {
        ...
        message.message = message.user + ' connected!'
        ...
        const avatarText = document.createTextNode(message.user)
        avatarElement.appendChild(avatarText);
        avatarElement.style['background-color'] = getAvatarColor(message.user)
        avatarContainer.appendChild(avatarElement)
        messageElement.style['background-color'] = getAvatarColor(message.user)
